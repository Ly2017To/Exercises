...
void Configurations::loadConfig() {
    ...
    for (const auto& camera : config["cameras"]) {
		...
		cfg.telegraphic = config["telegraphic"];
		cfg.aeiStation = config["aeiStation"];
		cfg.type = camera["type"];
		cfg.code = camera["code"];
		cfg.path = camera["path"];
		cfg.ip = camera["ip"];
...
std::vector<std::shared_ptr<Camera>> Configurations::createCameras() {
    ...
    for (const auto& cfg : cameraConfigs) {
        ...
        if (camera) {
			camera->setCameraParams(cfg.name, cfg.enable, cfg.telegraphic, cfg.aeiStation,cfg.type, cfg.code, cfg.path, cfg.ip, cfg.port,cfg.username, cfg.password, cfg.folder);
